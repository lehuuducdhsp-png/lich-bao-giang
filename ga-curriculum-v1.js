...
(function(root,factory){
  ...
  function previousGa(cat,current){const seq=sequence(cat);const i=seq.indexOf(Number(current));return i>0?seq[i-1]:null}
  ...
  // Gộp cùng một sự kiện thực dạy giữa GV KNS và CTV (hoặc nhiều GV cùng loại), để GA chỉ tiến 1 lần.
  function cooperativeKey(e){
    const cat=category(e.role||e.actualCategory),members=[...(e.members||[])].map(keyText).filter(Boolean).sort().join('+');
    return [e.sheet||'',e.locationKey||fold(e.schoolName||e.school),e.dateKey||'',e.day||'',e.session||'',Number(e.teachingPeriod??e.period)||'',cat,members].join('|')
  }
  function coalesce(events){
    const map=new Map();
    for(const src of events||[]){
      const e={...src,actualCategory:category(src.role||src.actualCategory)},k=cooperativeKey(e);
      if(!map.has(k))map.set(k,{...e,source:[...(e.source||[])],addresses:[...(e.addresses||[])],teacherCodes:[e.code].filter(Boolean),teacherNames:[e.teacherName].filter(Boolean),roles:[e.role].filter(Boolean)});
      else{
        const g=map.get(k);g.source.push(...(e.source||[]));g.addresses.push(...(e.addresses||[]));
        if(e.code&&!g.teacherCodes.includes(e.code))g.teacherCodes.push(e.code);
        if(e.teacherName&&!g.teacherNames.includes(e.teacherName))g.teacherNames.push(e.teacherName);
        if(e.role&&!g.roles.includes(e.role))g.roles.push(e.role);
        g.makeUp=g.makeUp||e.makeUp;
      }
    }
    return [...map.values()].sort((a,b)=>(a.date?.getTime?.()||0)-(b.date?.getTime?.()||0)||((a.session==='Sáng'?0:1)-(b.session==='Sáng'?0:1))||Number(a.teachingPeriod??a.period)-Number(b.teachingPeriod??b.period)||Number(a.row)-Number(b.row)||Number(a.col)-Number(b.col))
  }

  function assignProgression(events){
    const states=new Map();
    for(const event of events||[]){
      const cat=event.actualCategory||category(event.role),members=event.members||[];event.actualCategory=cat;event.previousByClass=[];event.historyMismatch=false;
      if(cat==='UNKNOWN'||!event.schoolName||!members.length){event.ga=null;event.note='Thiếu loại giáo viên, trường hoặc lớp nên chưa thể gợi ý GA.';continue}
      const memberStates=members.map(member=>{const key=`${fold(event.schoolName)}|${keyText(member)}|${cat}`;if(!states.has(key))states.set(key,{ga:null,events:[]});return{member,key,state:states.get(key)}});
      const candidates=memberStates.map(x=>nextGa(cat,x.state.ga));
      const valid=candidates.filter(x=>x!=null);event.ga=valid.length?valid.reduce((best,x)=>sequence(cat).indexOf(x)>sequence(cat).indexOf(best)?x:best,valid[0]):null;
      event.historyMismatch=new Set(candidates.map(String)).size>1;event.expectedCategory=cat;
      ...
      for(const item of memberStates){item.state.ga=event.ga;item.state.events.push(event)}
    }
    return events
  }

  return{version:'20260909.1',KNS_SEQUENCE,STEM_SEQUENCE,TITLES,category,sequence,isGaFor,nextGa,previousGa,title,gradesOf,cooperativeKey,coalesce,assignProgression};
});

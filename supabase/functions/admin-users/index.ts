...
const tempPasswordForUsername = (value: unknown) => {
  const match = cleanUsername(value).match(/^gv(\d+)$/)
  return match ? `hoannang${match[1]}` : ''
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      const policyPassword = tempPasswordForUsername(username)
      const password = policyPassword || String(body.password || '')
      ...
      return json({ ok: true, user_id: created.user.id, username, temp_password_policy: Boolean(policyPassword) })
    }

    if (action === 'reset_pending_gv_passwords') {
      ...
      for (const target of targets || []) {
        const username = cleanUsername(target.username)
        const password = tempPasswordForUsername(username)
        if (!password) continue
        const { error } = await admin.auth.admin.updateUserById(String(target.id), { password })
        if (error) {
        ...
        const { error: flagError } = await admin.from('profiles')
          ...
          .eq('id', target.id)
        if (flagError) {
          failed.push({ username, error: flagError.message })
          continue
        }
        reset.push(username)
      }

      return json({ ok: failed.length === 0, total: reset.length + failed.length, reset, failed }, failed.length ? 207 : 200)
    }
    ...
    if (action === 'reset_password') {
      if (targetProfile?.role === 'owner') return json({ error: 'Không đặt lại mật khẩu tài khoản chủ sở hữu bằng chức năng này.' }, 400)
      const policyPassword = tempPasswordForUsername(targetProfile?.username)
      const password = policyPassword || String(body.password || '')
      if (password.length < 8) return json({ error: 'Mật khẩu tạm phải có ít nhất 8 ký tự.' }, 400)
      ...
      return json({ ok: true, temp_password_policy: Boolean(policyPassword) })
    }
    ...
    if (action === 'delete') {
      if (targetProfile?.role === 'owner') return json({ error: 'Không thể xóa tài khoản chủ sở hữu.' }, 400)

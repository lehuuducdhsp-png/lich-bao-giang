...
-- CHECK-IN V1/V2 ROLLBACK ONLY.
-- Removes only Check-in objects created by the 2026-08-10 migrations.
-- It intentionally does not modify existing TKB, Auth, Storage, groups, reports, payroll,
-- or generic user_notifications objects.
-- BACK UP THE DATABASE BEFORE RUNNING ON PRODUCTION.

revoke execute on function public.request_teaching_checkin_quota(uuid) from authenticated;
...
revoke execute on function public.checkin_group_leader_can_target(uuid,uuid) from authenticated;
...
drop function if exists public.request_teaching_checkin_quota(uuid);
...
drop function if exists public.checkin_group_leader_can_target(uuid,uuid);
drop function if exists public.checkin_is_active_manager(uuid);
drop function if exists public.checkin_school_year(date);

drop table if exists public.teaching_checkin_quota_requests;

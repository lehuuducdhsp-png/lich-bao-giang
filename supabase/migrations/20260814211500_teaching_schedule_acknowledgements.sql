...
-- Xác nhận đã xem lịch dạy ngày mai.
-- Tách hoàn toàn khỏi Check-in GPS và không sửa dữ liệu lịch sử Check-in.
...
begin
  ...
  if btrim(coalesce(v_profile.teacher_code,''))='' then
    raise exception 'Tài khoản chưa liên kết mã giáo viên trong TKB.';
  end if;

  if p_teaching_date is distinct from (v_today + 1) then
    raise exception 'Chỉ xác nhận lịch dạy của ngày mai.';
  end if;

  if btrim(coalesce(p_schedule_signature,''))='' or char_length(p_schedule_signature)>20000 then
    raise exception 'Dấu nhận diện lịch dạy không hợp lệ.';
  end if;
  if char_length(coalesce(p_schedule_summary,''))>5000 then
    raise exception 'Nội dung tóm tắt lịch dạy quá dài.';
  end if;

  insert into public.teaching_schedule_acknowledgements(
    user_id,teaching_date,schedule_signature,schedule_summary,
    source_tkb_id,source_tkb_updated_at,acknowledged_at,updated_at
  ) values(
    v_uid,p_teaching_date,p_schedule_signature,coalesce(p_schedule_summary,''),
    p_source_tkb_id,p_source_tkb_updated_at,now(),now()
  )
...
create or replace function public.my_teaching_schedule_ack(
  p_teaching_date date
)
returns jsonb
language plpgsql
...
begin
  ...
  return jsonb_build_object(
    'teaching_date',v_row.teaching_date,
    'schedule_signature',v_row.schedule_signature,
    'schedule_summary',v_row.schedule_summary,
    'source_tkb_id',v_row.source_tkb_id,
    'source_tkb_updated_at',v_row.source_tkb_updated_at,
    'acknowledged_at',v_row.acknowledged_at,
...
begin
  if not public.is_owner() then
    raise exception 'Chỉ Chủ sở hữu được cấp quyền theo dõi xác nhận lịch dạy.';
  end if;
  ...
  if not found then
    raise exception 'Chỉ có thể cấp quyền này cho tài khoản Quản lý đang hoạt động.';
  end if;
end;
$$;
...
begin
  ...
  select exists(
    ...
    where p.id=v_uid
      and p.is_active
      and p.is_manager
      and p.manager_can_schedule_ack_monitor
      and (p.expires_at is null or p.expires_at>now())
  ) into v_manager_monitor;

  if not v_owner and not v_leader and not v_manager_monitor then

...
(function(){
  ...
  function install(){
    ...
    api.analyze=analyze;window.analyzeNow=analyze;try{document.dispatchEvent(new CustomEvent('lbg-analyze-now-ready',{detail:{source:'tkb-parser-school-name-fix-v1'}}))}catch{};api.__lbgSchoolNameFixV1=true;
    const btn=document.getElementById('analyze');if(btn){btn.dataset.lbgParserV2='1';btn.onclick=function(){const sel=document.getElementById('teacher'),week=document.getElementById('week'),opt=sel?.selectedOptions?.[0],code=sel?.value||'',name=opt?.dataset?.name||code,old=btn.textContent;btn.disabled=true;btn.textContent='Đang kiểm tra…';try{const localBook=typeof wb!=='undefined'?wb:null,ws=localBook?.getWorksheet(week?.value);const runner=typeof window.analyzeNow==='function'?window.analyzeNow:analyze;window.result=runner(ws,code,name);window.render?.(window.result);const ex=document.getElementById('export');if(ex)ex.disabled=!window.result.total;if(typeof window.toast==='function')window.toast(`Đã kiểm tra ${window.result.total} tiết.`)}catch(error){console.error(error);alert('Không kiểm tra được lịch: '+(error?.message||error))}finally{btn.textContent=old;btn.disabled=!sel?.value}}}

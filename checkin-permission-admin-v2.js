...
(function(){
  ...
  function stateOfRow(el){return{
    can_checkin:Boolean(el.querySelector('[data-perm-checkin]')?.checked),
    can_review:Boolean(el.querySelector('[data-perm-review]')?.checked),
    note:txt(el.querySelector('[data-perm-note]')?.value)
  }}
  ...
  function buckets(){
    ...
    list.sort((a,b)=>{
      const rank=x=>x.key==='admin'?0:x.key==='ungrouped'?2:1;
      return rank(a)-rank(b)||a.name.localeCompare(b.name,'vi');
    });
    return list
  }

  function selectedBucket(){const sel=q('lbgPermBucket');return sel?.value||''}
  function renderRows(){
    ...
    body.innerHTML=rows.map(r=>{
      ...
      return`<tr data-perm-user="${esc(r.user_id)}" data-bucket="${esc(r.bucket_key)}" data-dirty="0">
        <td><b>${esc(r.display_name)}</b><div class="lbg-perm-sub">${esc(r.username)}${r.teacher_code?` • Mã ${esc(r.teacher_code)}`:' • Chưa có mã TKB'}</div></td>
        <td data-label="Vai trò"><span class="lbg-perm-role">${esc(roleName(r))}</span></td>
        <td data-label="Check-in" title="${esc(checkTitle)}"><input type="checkbox" data-perm-checkin ${r.can_checkin?'checked':''} ${checkDisabled?'disabled':''}></td>
        <td data-label="Kiểm tra" title="${esc(reviewTitle)}"><input type="checkbox" data-perm-review ${r.can_review?'checked':''} ${reviewDisabled?'disabled':''}></td>
        <td data-label="Ghi chú"><input type="text" data-perm-note maxlength="200" value="${esc(r.note||'')}" placeholder="Tùy chọn"></td>
      </tr>`
    }).join('');
  ...
  function applyFilter(){
    ...
    document.querySelectorAll('#lbgPermBody [data-perm-user]').forEach(tr=>{
      const same=!bucket||tr.dataset.bucket===bucket;total+=same?1:0;
      const match=!needle||txt(tr.textContent).toLocaleLowerCase('vi').includes(needle);
      const show=same&&match;tr.hidden=!show;if(show)visible++
    });
    const meta=q('lbgPermVisible');if(meta)meta.textContent=`Hiển thị ${visible}/${total} tài khoản trong mục đang chọn.`;
    const empty=q('lbgPermEmpty');if(empty)empty.hidden=visible>0
  }

  function dirtyRows(){
    return [...document.querySelectorAll('#lbgPermBody [data-perm-user]')].filter(tr=>original.get(String(tr.dataset.permUser))!==stateKey(stateOfRow(tr)))
  }
  ...
  function renderPanel(){
    ...
    let panel=q('lbgCheckinPermissionAdminV2');
    if(!panel){panel=document.createElement('div');panel.id='lbgCheckinPermissionAdminV2';panel.className='lbg-checkin-panel lbg-checkin-wide';grid.appendChild(panel)}
    ...
    panel.innerHTML=`<div class="lbg-perm-head"><div><div class="lbg-perm-title"><h4>⚙️ Quản lý quyền Check-in</h4><span class="lbg-perm-pilot" title="PILOT là giai đoạn thử nghiệm giới hạn trước khi mở chính thức">PILOT • THỬ NGHIỆM</span></div><p style="margin:5px 0 0">${owner?'PILOT nghĩa là hệ thống đang thử nghiệm với một nhóm tài khoản giới hạn trước khi mở chính thức. Chủ sở hữu quản lý toàn hệ thống theo từng nhóm.':'Bạn chỉ bật/tắt Check-in cho giáo viên thường thuộc nhóm mình. Quyền “Kiểm tra” và phạm vi ngoài nhóm vẫn do Chủ sở hữu quản lý.'}</p></div><span id="lbgPermDirty" class="lbg-perm-unsaved" hidden></span></div>
      <div class="lbg-checkin-note" style="margin-top:10px"><b>Cách dùng gọn khi có nhiều giáo viên:</b> chọn <b>Nhóm 1, Nhóm 2, Nhóm 3…</b> hoặc <b>Hành chính / Quản lý</b>, tìm tên nếu cần, chỉnh nhiều người rồi bấm <b>Lưu tất cả thay đổi</b> một lần.</div>
      <div class="lbg-perm-toolbar"><label>Phạm vi<select id="lbgPermBucket">${b.map(x=>`<option value="${esc(x.key)}" ${x.key===remembered?'selected':''}>${esc(x.name)} (${x.count})</option>`).join('')}</select></label><label>Tìm giáo viên<input id="lbgPermSearch" type="search" placeholder="Tên, tài khoản hoặc mã TKB"></label><button class="btn outline" id="lbgPermUndo" disabled>Hoàn tác</button><button class="btn primary" id="lbgPermSaveAll" disabled>Lưu tất cả thay đổi</button></div>
      <div class="lbg-perm-summary"><span id="lbgPermVisible" class="meta"></span>${owner?'<span class="meta">• “Kiểm tra” chỉ bật cho Nhóm trưởng/Hành chính phù hợp.</span>':'<span class="meta">• Nhóm trưởng không thể tự cấp quyền “Kiểm tra” cho thành viên.</span>'}</div>
      <div class="lbg-perm-list"><table class="lbg-perm-table"><thead><tr><th>Tài khoản</th><th>Vai trò</th><th>Check-in</th><th>Kiểm tra</th><th>Ghi chú</th></tr></thead><tbody id="lbgPermBody"></tbody></table><div id="lbgPermEmpty" class="lbg-perm-empty" hidden>Không có tài khoản phù hợp.</div></div>`;
    q('lbgPermBucket')?.addEventListener('change',e=>{sessionStorage.setItem(storageKey,e.target.value);applyFilter()});
    q('lbgPermSearch')?.addEventListener('input',applyFilter);
    q('lbgPermSaveAll')?.addEventListener('click',saveAll);
    q('lbgPermUndo')?.addEventListener('click',undoAll);
    renderRows();
    const old=findOldPilotPanel();if(old&&old!==panel)old.style.display='none';
    return true
  }

  function undoAll(){
    for(const tr of document.querySelectorAll('#lbgPermBody [data-perm-user]')){
      const r=rows.find(x=>String(x.user_id)===String(tr.dataset.permUser));if(!r)continue;
      const c=tr.querySelector('[data-perm-checkin]'),v=tr.querySelector('[data-perm-review]'),n=tr.querySelector('[data-perm-note]');
      if(c)c.checked=Boolean(r.can_checkin);if(v)v.checked=Boolean(r.can_review);if(n)n.value=r.note||''
    }
  ...
  async function saveAll(){
    ...
    try{
      const payload=changed.map(tr=>({user_id:tr.dataset.permUser,...stateOfRow(tr)}));
      const result=await rpc('set_checkin_pilot_access_bulk',{p_changes:payload});
      button.textContent=`Đã lưu ${Number(result?.changed_count)||payload.length} thay đổi ✓`;
      await load(true);
      setTimeout(()=>{const b=q('lbgPermSaveAll');if(b&&!dirtyRows().length)b.textContent='Lưu tất cả thay đổi'},900)
    }catch(error){button.disabled=false;button.textContent=old;alert('Không lưu được quyền Check-in: '+(error?.message||String(error)))}
  }

  async function load(force=false){
    if(loading||!eligibleViewer()||!window.LBGAuth?.client)return;loading=true;
    try{
      gate=await rpc('checkin_access_context');
      if(gate?.phase!=='pilot')return;
      const data=await rpc('checkin_permission_admin_list');rows=Array.isArray(data)?data:[];
      style();renderPanel()
    }catch(error){console.warn('Quản lý quyền Check-in V2 chưa sẵn sàng:',error?.message||error)}
    finally{loading=false}
  }

  function queue(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;if(rows.length&&gate?.phase==='pilot')renderPanel();else load().catch(console.error)})}
  ...
  window.addEventListener('focus',()=>load(true).catch(console.error));

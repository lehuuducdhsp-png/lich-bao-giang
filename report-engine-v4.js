...
(function(){
  ...
  const GA_PREFIX='lbgGaManualV2',LEGACY_GA_PREFIX='lbgGaManualV1',selection=new Map(),sundayCache=new WeakMap();
  ...
  function weekHasSunday(ws){
    if(!ws)return false;if(sundayCache.has(ws))return sundayCache.get(ws);
    let found=false;
    try{
      const parser=window.LBGTkbParserV2;
      if(parser?.scanAssignments)found=(parser.scanAssignments(ws)||[]).some(e=>Number(e.day)===8);
      else{
        const codes=new Set(readTeachers(ws,true).map(x=>txt(x?.code).toUpperCase()).filter(Boolean));
        if(codes.size)ws.eachRow({includeEmpty:false},row=>{if(found)return;for(let c=65;c<=74;c++){const value=txt(row.getCell(c).text).toUpperCase();if(value&&codes.has(value)){found=true;break}}})
      }
    }catch{}
  ...
  function locationList(a,d,s){
    ...
    for(const e of(a?.entries||[])){
      if(Number(e.day)!==Number(d)||e.session!==s)continue;
      const loc=locOf(e);if(!loc.key&&!loc.label)continue;
      if(!map.has(loc.key||loc.label))map.set(loc.key||loc.label,loc)
    }
    return[...map.values()]
  }

  function reportKey(a,prefix=GA_PREFIX){return`${prefix}:${activeVersion()}:${txt(a?.sheet)}:${txt(a?.code||a?.teacherName)}`}
  function ensureGa(a){
    if(!a)return{};if(a.gaValues&&typeof a.gaValues==='object')return a.gaValues;
    try{
      const current=JSON.parse(localStorage.getItem(reportKey(a))||'null');
      if(current&&typeof current==='object'){a.gaValues=current;return a.gaValues}
      const legacy=JSON.parse(localStorage.getItem(reportKey(a,LEGACY_GA_PREFIX))||'{}');
      a.gaValues=legacy&&typeof legacy==='object'?legacy:{}
    }catch{a.gaValues={}}
    return a.gaValues
  }
  const gaKey=(d,s,loc)=>`${Number(d)}|${txt(s)}|${txt(loc?.key||loc?.label)}`,legacyGaKey=(d,s,loc)=>`${Number(d)}|${txt(s)}|${txt(loc?.legacySchool||loc?.schoolName)}`;
  function gaValue(a,d,s,loc){
    const data=ensureGa(a),newKey=gaKey(d,s,loc);
    if(data[newKey]!=null)return String(data[newKey]);
    const old=data[legacyGaKey(d,s,loc)];
  ...
  function style(){
    if(q('lbgReportEngineV4Css'))return;
    const s=document.createElement('style');s.id='lbgReportEngineV4Css';s.textContent=`
      .lbg-r4-help{margin:0 0 10px;padding:9px 11px;border:1px solid #bfdbfe;border-radius:11px;background:#eff6ff;color:#1e40af;font-size:12px}
      .lbg-r4-sunday{display:inline-flex;margin-left:7px;padding:3px 7px;border:1px solid #fed7aa;border-radius:999px;background:#fff7ed;color:#9a3412;font:800 10px system-ui,sans-serif;vertical-align:middle}
      .lbg-r4-location{display:grid;justify-items:center;gap:2px;padding:4px 2px}
  ...
  function addReportSheet(bookOut,a,name){
    ...
    const end=endDate(a.start,sun);if(a.start&&end)ws.getCell('A3').value=`(Từ ngày ${a.start.toLocaleDateString('vi-VN')} đến ngày ${end.toLocaleDateString('vi-VN')})`;
    ws.getRow(4).values=['Buổi','Tiết',...ds.map(dayLabel)];
    for(const[session,sr,pr]of[['Sáng',5,6],['Chiều',11,12]]){
      ws.getCell(sr,1).value=session;ws.getCell(sr,2).value='Tiết';for(let p=1;p<=5;p++)ws.getCell(pr+p-1,2).value='Tiết '+p;
      ds.forEach((d,i)=>{
  ...
  function selectedTeachers(){
    ...
    const allowed=new Map(readTeachers(ws,false).map(x=>[txt(x.code).toUpperCase(),{code:txt(x.code),name:txt(x.name||x.teacherName||x.code)}])),picked=[...selection.values()].filter(x=>allowed.has(txt(x.code).toUpperCase()));
    if(picked.length)return picked;
    const code=txt(q('teacher')?.value),opt=q('teacher')?.selectedOptions?.[0];if(code)return[{code,name:txt(opt?.dataset?.name||allowed.get(code.toUpperCase())?.name||code)}];
  ...
  async function exportUnified(){
    const button=q('export');if(!button)return;const old=button.textContent;button.disabled=true;button.textContent='Đang tạo file…';
    try{

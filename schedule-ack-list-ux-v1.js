...
(function(){
  ...
  function clampPage(state,total){
    const pages=Math.max(1,Math.ceil(total/state.size));
    state.page=Math.min(Math.max(1,state.page),pages);
    return pages;
  }
  function rangeText(page,size,total){
    if(!total)return'0–0/0';
    const start=(page-1)*size+1,end=Math.min(total,page*size);
    return`${start}–${end}/${total}`;
  }
  function pagerHtml(prefix,state,total,pages){
    return `<div class="lbg-ack-ux-pager" data-ack-ux-pager="${prefix}"><label>Hiển thị<select data-ack-ux-size="${prefix}"><option value="10" ${state.size===10?'selected':''}>10 dòng/trang</option><option value="20" ${state.size===20?'selected':''}>20 dòng/trang</option><option value="50" ${state.size===50?'selected':''}>50 dòng/trang</option></select></label><span class="lbg-ack-ux-range">${rangeText(state.page,state.size,total)}</span><button class="btn outline mini lbg-ack-ux-nav" data-ack-ux-prev="${prefix}" ${state.page<=1?'disabled':''}>‹</button><span class="lbg-ack-ux-pageinfo">Trang ${state.page}/${pages}</span><button class="btn outline mini lbg-ack-ux-nav" data-ack-ux-next="${prefix}" ${state.page>=pages?'disabled':''}>›</button></div>`;
  }

  function wirePager(host,prefix,state,applyFn){
  ...
  function enhancePermission(){
    ...
    const list=box.querySelector('.lbg-ack-perm-list');if(!list)return;
    const state=states.permission;
    let toolbar=box.querySelector('[data-ack-ux-toolbar="permission"]');
    if(!toolbar){
      toolbar=document.createElement('div');toolbar.className='lbg-ack-ux-toolbar';toolbar.dataset.ackUxToolbar='permission';
      ...
      const input=toolbar.querySelector('[data-ack-ux-query="permission"]'),status=toolbar.querySelector('[data-ack-ux-status="permission"]');
      input.value=state.query;status.value=state.status;
      input.oninput=()=>{state.query=input.value;state.page=1;applyPermission()};
      status.onchange=()=>{state.status=status.value;state.page=1;applyPermission()};
    }
    applyPermission();
  }
  ...
  function applyPermission(){
    ...
    const state=states.permission,rows=[...list.querySelectorAll('.lbg-ack-perm-row')];
    const enabledTotal=rows.filter(r=>r.querySelector('input[type="checkbox"]')?.checked).length;
    ...
    const filtered=rows.filter(row=>{
      const cb=row.querySelector('input[type="checkbox"]'),enabled=Boolean(cb?.checked);
      if(state.status==='enabled'&&!enabled)return false;
      if(state.status==='disabled'&&enabled)return false;
      if(wanted&&!fold(row.textContent).includes(wanted))return false;
      return true;
    });
    const pages=clampPage(state,filtered.length),start=(state.page-1)*state.size,end=start+state.size,visible=new Set(filtered.slice(start,end));
    rows.forEach(r=>r.hidden=!visible.has(r));
    box.querySelector('[data-ack-ux-pager="permission"]')?.remove();
    list.insertAdjacentHTML('afterend',pagerHtml('permission',state,filtered.length,pages));
    box.querySelector('[data-ack-ux-noresult="permission"]')?.remove();
    if(!filtered.length)list.insertAdjacentHTML('afterend','<div class="lbg-ack-ux-no-result" data-ack-ux-noresult="permission">Không có tài khoản phù hợp bộ lọc.</div>');
  ...
  function enhanceMonitor(box){
    ...
    input.value=state.query;group.value=state.group;status.value=state.status;
    if(!toolbar.dataset.bound){toolbar.dataset.bound='1';input.oninput=()=>{state.query=input.value;state.page=1;applyMonitor(box)};group.onchange=()=>{state.group=group.value;state.page=1;applyMonitor(box)};status.onchange=()=>{state.status=status.value;state.page=1;applyMonitor(box)}}
    applyMonitor(box);
  }
  ...
  function applyMonitor(box){
    ...
    const filtered=rows.filter(row=>{
      if(state.status!=='all'&&rowStatus(row)!==state.status)return false;
      const group=txt(row.cells?.[1]?.textContent);if(state.group!=='all'&&group!==state.group)return false;
      if(wanted&&!fold(row.textContent).includes(wanted))return false;
      return true;
    });
    const pages=clampPage(state,filtered.length),start=(state.page-1)*state.size,end=start+state.size,visible=new Set(filtered.slice(start,end));
    rows.forEach(r=>r.hidden=!visible.has(r));
    box.querySelector(`[data-ack-ux-pager="${prefix}"]`)?.remove();
    wrap.insertAdjacentHTML('afterend',pagerHtml(prefix,state,filtered.length,pages));
    box.querySelector(`[data-ack-ux-noresult="${prefix}"]`)?.remove();
    if(!filtered.length)wrap.insertAdjacentHTML('afterend',`<div class="lbg-ack-ux-no-result" data-ack-ux-noresult="${prefix}">Không có giáo viên phù hợp bộ lọc.</div>`);
  ...
  function apply(){
    style();
    enhancePermission();
    const main=q('lbgAckMonitorV2');if(main)enhanceMonitor(main);
    const flexible=q('lbgAckMonitorFlexible');if(flexible)enhanceMonitor(flexible);
  }

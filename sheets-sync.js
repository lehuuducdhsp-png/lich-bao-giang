...
(function(){
  ...
  function yearConfig(){
    ...
    const end=parseLocalDate(cfg.endDate);
    const valid=Boolean(
      Number.isInteger(yearStart)&&start&&end&&start.getDay()===1&&end>=start&&start.getFullYear()===yearStart
    );
  ...
  function buildReport(mode){
    ...
    if(!cfg.valid){
      throw new Error('Cấu hình năm học chưa hợp lệ. Ngày bắt đầu Tuần 01 phải là Thứ Hai và ngày kết thúc phải ở phía sau.');
    }

    const yearStart=cfg.yearStart;
    ...
    const destinationSheet=tabName(weekText,start,a.week,yearStart,cfg);
    ...
    return {
      ...
      schoolYearStartDate:cfg.startDate,
      schoolYearEndDate:cfg.endDate,
      yearStart,
      yearEnd:yearStart+1,
  ...
  function ensureButton(){
    ...
    Object.assign(button,{
      ...
      className:'btn',
      disabled:true,
      textContent:'☁ Lưu vào Google Sheets'
    });
    ...
    const sync=()=>{
      ...
      const a=current();
      const cfg=yearConfig();
      button.disabled=!a||!Array.isArray(a.entries)||a.entries.length<1||exportButton.disabled||!cfg.valid;
    };
    ...
    document.getElementById('year')?.addEventListener('change',()=>setTimeout(sync,50));
    document.getElementById('schoolYearStartDate')?.addEventListener('change',sync);
    document.getElementById('schoolYearEndDate')?.addEventListener('change',sync);
    setInterval(sync,1000);
    sync();
  }

  function ensureDialog(){
    let d=document.getElementById('sheetSaveDialog');
    if(d)return d;

    d=document.createElement('dialog');
    d.id='sheetSaveDialog';
    ...
    d.innerHTML=`<form method="dialog" style="padding:22px;font:15px/1.5 system-ui,-apple-system,Segoe UI,sans-serif">
    ...
    d.addEventListener('close',()=>{
      if(['overwrite','copy'].includes(d.returnValue))saveToSheets(d.returnValue);
    });
    return d;
  }
  ...
  function chooseMode(){
    try{
      ...
      const p=document.getElementById('sheetSavePrompt');
      if(p){
        p.textContent=`${r.schoolYear} • ${r.sheetName} • ${r.teacherName} • ${r.total} tiết • ${Object.keys(r.gaValues).length} ô GA`;
      }
      d.returnValue='cancel';
      typeof d.showModal==='function'
        ?d.showModal()
        :saveToSheets(confirm('Nhấn OK để ghi đè. Nhấn Hủy để tạo BẢN 2.')?'overwrite':'copy');
    }catch(e){
  ...
  function setBusy(busy,text){
    const b=document.getElementById('saveSheets');
    if(!b)return;
    const a=current();
    const cfg=yearConfig();
    b.disabled=busy||!a?.entries?.length||!cfg.valid;
    b.textContent=text||(busy?'Đang lưu…':'☁ Lưu vào Google Sheets');
  }
  ...
  function saveToSheets(mode){
    ...
    f.append(
      ...
      hidden('accessKey',ACCESS_KEY),
      hidden('payload',JSON.stringify(report))
    );
    ...
    try{
      ...
      f.remove();
      finish(false,'Không gửi được dữ liệu sang Google Sheets: '+(e.message||String(e)));
      return;
    }
    f.remove();
  ...
  function finish(ok,message,url){
    ...
    pending=null;
    setBusy(false,ok?'✓ Đã lưu Google Sheets':'☁ Lưu vào Google Sheets');

    if(ok){
      toast(message||'Đã lưu vào Google Sheets.');
      if(url&&confirm((message||'Đã lưu thành công.')+'\n\nMở Google Sheets ngay?')){
  ...
  window.addEventListener('message',event=>{
    const host=(()=>{
      try{return new URL(event.origin).hostname;}
      catch{return '';}
    })();
    ...
    const m=event.data||{};
    if(!pending||(m.requestId&&m.requestId!==pending.requestId))return;

    const body=m.result||m.data||m.payload||m;
    const err=m.type==='LBG_ERROR'||m.success===false||body?.success===false||Boolean(m.error||body?.error);
    const ok=m.type==='LBG_SAVED'||m.type==='LBG_SAVE_RESULT'||m.type==='LBG_RESULT'||m.success===true||body?.success===true||body?.ok===true;

    if(err){
      finish(false,String(m.error||body?.error||body?.message||'Apps Script báo lỗi khi lưu.'));
    }else if(ok){
      finish(
        true,
        String(body?.message||m.message||'Đã lưu vào Google Sheets.'),
        body?.url||body?.spreadsheetUrl||m.url
      );
    }
  });

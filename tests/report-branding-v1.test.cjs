...
assert.match(src,/getCell\(1,rightStart\)/,'Tên trung tâm phải ghi vào hàng 1, cột bắt đầu bên phải logo (C1), không được rơi vào vùng A1:B3.');
assert.doesNotMatch(src,/getCell\(rightStart,1\)/,'Không được đảo row/column làm tên trung tâm ghi vào A3/A1 dưới logo.');
...
assert.match(patchLoader,/report-branding-v1\.js\?v=20260925\.2/,'Patch loader phải nạp đúng bản sửa merge mới.');
